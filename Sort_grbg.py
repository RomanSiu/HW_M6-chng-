...
def normalize(text, suf = None):
    ...
    text = text[:-(len(suf))] if suf else text
...
def sorting(path):
    ...
    for file in path.iterdir():
        if file.is_dir() and file.name not in folder_list:
            sorting(file)
            file.rmdir()
            continue
        ...
        if suf in suf_dict['img']:
            ...
            inc_suf.append(suf) if suf not in inc_suf else ...
            ...
            unknown_suf.append(suf) if suf not in inc_suf else ...
